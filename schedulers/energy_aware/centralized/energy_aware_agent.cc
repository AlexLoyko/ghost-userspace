...
namespace ghost {
  ...
  void ParseEnergyAwareConfig(EnergyAwareConfig* config) {
    CpuList ghost_cpus =
...
int main(int argc, char* argv[]) {
  ...
  ghost::EnergyAwareConfig config;
  ghost::ParseEnergyAwareConfig(&config);
  ...
  auto uap = new ghost::AgentProcess<ghost::FullEnergyAwareAgent<ghost::LocalEnclave>,
      ghost::EnergyAwareConfig>(config);
  ...
  ghost::GhostSignals::AddHandler(SIGUSR1, [uap](int) {
    uap->Rpc(ghost::EnergyAwareScheduler::kDebugRunqueue);
    return false;
  });
